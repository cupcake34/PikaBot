...
exports.run = (client, message, args) => {
  ...
  message.channel.send(`**List Of Commands in ${args[0]}**\`\`\`${client.commands.filter(c => c.help.module === args[0]).map(cmd => `${config.prefix}${cmd.help.name}${' '.repeat(longest - cmd.help.name.length)} :: ${cmd.help.description}`)\`\`\``);
